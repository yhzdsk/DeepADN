...
class AudioFeaturizer(nn.Module):
    ...
    def feature_dim(self):
        ...
        if self._feature_method == 'MelSpectrogram':
            n_mels = self._method_args.get('n_mels', 128)  # 获取 n_mels，默认为 40
            return n_mels  # 返回 n_mels 和 f_max
            ...
            n_mfcc = self._method_args.get('n_mfcc', 40)  # 获取 n_mfcc，默认为 40
            melkwargs = {
                'n_mels': self._method_args.get('n_mels', 40),
                'f_min': self._method_args.get('f_min', 0),      # 获取 f_min，默认为 0
                'f_max': self._method_args.get('f_max', 3000)     # 获取 f_max，默认为 4000
                  }
            return n_mfcc, melkwargs  # 返回 n_mfcc 和 melkwargs
